...
def convert_full_conv():
    mm= 'lightcnn/binarylastinnerproduct_l_sony__iter_good2.caffemodel'
 # Load the original network and extract the fully connected layers' parameters.
    net = caffe.Net('/home/pub/Work/face_verification_experiment/proto/Binary_LightenedCNN_B_deploy.prototxt.laskinnerproduct',
                    mm,
                    caffe.TEST)
    print net.params
...
def face_detection(imgList):
  ...
  for imgFile in open(imgList).readlines():
	...
	max = img.size[0]
        ...
        min = img.size[0]
	max = img.size[1]
	print 'size1:', img.size[0], img.size[1]
    delim = 2500/max
    if(delim == 1):
	scales.append(1)
    elif(delim > 1):
	...
	print 'size2:', scale_img.size[0], scale_img.size[1]
